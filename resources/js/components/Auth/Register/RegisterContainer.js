...
handleSubmit(e) {  
 ...
 }).catch(error => {if (error.response) {debugger;
...
<div class="card-header text-center" ><h3>Create Your Account</h3></div>
<div class="card-body">
<div className="row">
        ...
        {this.state.error ? <FlashMessage duration={900000} persistOnHover={true}>
          ...
          <form onSubmit={this.handleSubmit}>
            <div className="form-group">
              <input id="c_name" type="text" name="c_name" placeholder="Name" className="form-control" required onChange={this.handleName}/>
            </div>
            <div className="form-group">
            ...
            <button type="submit" name="singlebutton" className="btn btn-primary btn-lg  btn-block mb10" disabled={this.state.formSubmitting ? "disabled" : ""}>Create Account</button>
          </form>
          <p className="text-dark mt-3">Already have an account?
            <Link to="/login" className="text-yellow"> Log In</Link> |
            <span className="pull-right"><Link to="/" className="text-dark">Back to Home</Link></span>
        </p>
      </div>
